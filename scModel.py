...
class Model:
    ...
    def HBdG(self):
        ...
        #h*
        H[np.array([6, 7, 9, 10]), np.array([7, 6, 10, 9])] = lambda kx, ky, delta: 2*self.t * np.cos(kx/2) # Some function of kx, ky
        H[np.array([7, 8, 10, 11]), np.array([8, 7, 11, 10])] = lambda kx, ky, delta: 2*self.t * np.cos(ky/2)
        
        #Del
        H[np.array([0,1,2, 6, 7, 8]), np.array([9, 10, 11, 3, 4, 5])] = lambda kx, ky, delta: delta
        H[np.array([3, 4, 5, 9, 10, 11]), np.array([6, 7, 8, 0, 1, 2])] = lambda kx, ky, delta: -delta
        ...
        def Hk(kx, ky): 
            ...
            eps = 1e-15
            for index in np.ndindex(H.shape): hk[index] = H[index](kx, ky, self.Del)
            hk[np.abs(hk) < eps] = 0
