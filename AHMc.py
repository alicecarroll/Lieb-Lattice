...
class Model:
    def __init__(self, **kwargs):
        ...
        #self.Del0Ac = kwargs.get('Del0Ac', 0.0)
        #self.Del0Bc = kwargs.get('Del0Bc', 0.0)
        #self.Del0Cc = kwargs.get('Del0Cc', 0.0)
        self.Del0Ac = np.conjugate(self.Del0A)
        self.Del0Bc = np.conjugate(self.Del0B)
        self.Del0Cc = np.conjugate(self.Del0C)
    ...
    def HBdG(self):
        ...
        H[np.array([9]), np.array([0])] = lambda kx, ky: -self.Del0Ac/2

        H[np.array([3]), np.array([6])] = lambda kx, ky: self.Del0A/2
        H[np.array([6]), np.array([3])] = lambda kx, ky: self.Del0Ac/2

        H[np.array([1]), np.array([10])] = lambda kx, ky: -self.Del0B/2
        H[np.array([10]), np.array([1])] = lambda kx, ky: -self.Del0Bc/2

        H[np.array([4]), np.array([7])] = lambda kx, ky: self.Del0B/2
        H[np.array([7]), np.array([4])] = lambda kx, ky: self.Del0Bc/2
        
        H[np.array([2]), np.array([11])] = lambda kx, ky: -self.Del0C/2
        H[np.array([11]), np.array([2])] = lambda kx, ky: -self.Del0Cc/2

        H[np.array([5]), np.array([8])] = lambda kx, ky: self.Del0C/2
        H[np.array([8]), np.array([5])] = lambda kx, ky: self.Del0Cc/2
    ...
    def DeltaN(self, k):
        ...
        Deltac=np.zeros((6,6), dtype=complex)
        Nu=np.zeros((6,6), dtype=complex)
        ...
        for x in k:
            for y in k:
                ...
                Carr = np.zeros((12,12), dtype=complex)
                for i in range(12):
                    Carr[i]=Evec[int(indexmap[i])]
                #Carr[np.abs(Carr)<eps]=0

                #print(Carr)
                u=Carr[0:6, 0:6]
                v=Carr[0:6,6:]   
                #print(u@np.conjugate(u.T)+v@np.conjugate(v.T))
                el =np.matmul(np.conjugate(u.T), v)
                el2 = np.matmul(np.conjugate(v.T), u)
        ...
        danc=np.abs(self.U)/le**2*Deltac[0,3]
        dbnc=np.abs(self.UB)/le**2*Deltac[1,4]
        dcnc=np.abs(self.U)/le**2*Deltac[2,5]

        #print('func1', [dan, dbn, dcn, danc, dbnc, dcnc])
        return [dan, dbn, dcn, danc, dbnc, dcnc], [nuA, nuB, nuC] 
        
    def Deltra(self, k, Nmax=20, Nmin=10, alpha=0):
        dan, dbn, dcn, danc, dbnc, dcnc = (self.Del0A, self.Del0B, self.Del0C, self.Del0Ac, self.Del0Bc, self.Del0Cc)
        delarrn = np.array([dan, dbn, dcn, danc, dbnc, dcnc])
        dels = delarrn.reshape(6,1)
        #print('1st:', delarrn)
        ...
        c=0
        while (c<Nmax and (np.abs(r2[-5:])>0.0002).any()) or c<Nmin:
        #for i in range(N):
            print(c, r2[-1])
            if c==5 or c%50==0:
            ...
            dels = np.concatenate((dels, delarro.reshape(6,1)), axis=1)
            ...
            self.Del0Ac =delarrn[3]
            self.Del0Bc = delarrn[4]
            self.Del0Cc = delarrn[5]
        ...
        dels = np.concatenate((dels, delarrn.reshape(6,1)), axis=1)
